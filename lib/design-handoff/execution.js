...
function createExecuteStage(ctx) {
  return function executeStage() {
    ...
    if (!blocked) {
      ...
      ctx.refreshProof(handoff, proof);
      if (phase.id === 'creative_reference' && artifact === 'visual.reference.request.json') {
        const next = 'Generate the ChatGPT/Image 2.0 reference from visual.reference.request.json, save visual.target.png, then approve creative_reference.';
        handoff.current_gate = next;
        proof.next_required_action = next;
      } else if (phase.id === 'asset_decomposition' && artifact === 'asset.extraction.request.json') {
        const next = 'Generate the requested Image 2.0 asset from asset.extraction.request.json, save it under assets/, update creative.asset-kit.json, then approve asset_decomposition.';
        handoff.current_gate = next;
        proof.next_required_action = next;
      }
      ctx.writeJson(files.handoff, handoff);
