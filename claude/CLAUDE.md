...
| "Manage Clerk: users/orgs/sessions, env keys, instance config, deploy status, Backend/Platform API calls" | `clerk-cli` | Operates the `clerk` binary — prefer over raw HTTP (handles auth, key resolution, app/instance targeting, formatting). |
...
| Code STRUCTURE of an indexed repo — find a symbol / its call-chains / routes / architecture WITHOUT re-reading files (sub-ms, ~10× fewer tokens) | `codebase-memory-mcp` *(user-scoped stdio binary `~/local/bin/codebase-memory-mcp`, ✓Connected; index a repo first: `codebase-memory-mcp cli index_repository --repo-path <abs>`; mym-autotrader indexed 2026-07-23)* | 15 tools (search_graph / trace_path / get_architecture / search_code / get_code_snippet / manage_adr…). **Prefer over grep+read** for "where is X / what calls X / repo shape" in the autonomous loops. Queries need `project` (from list_projects), not repo_path. Verified SHA-256, MIT, 100% local; ~0.5GB graph/repo — mind disk. See [[reference-codebase-memory-mcp]]. |
