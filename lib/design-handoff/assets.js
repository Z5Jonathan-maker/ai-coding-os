    ...
    if (!ctx.has('image-api-ok')) {
      ...
      writeJson(path.join(dir, 'visual.reference.request.json'), {
        ...
        instructions: [
          ...
          `Then run cc-design-handoff approve --dir ${JSON.stringify(dir)} --phase creative_reference --artifact visual.target.png.`,
        ],
        prompt,
      });
      return '';
    ...
    if (!ctx.has('image-api-ok')) {
      writeJson(path.join(dir, 'asset.extraction.request.json'), {
        ...
        instructions: [
          ...
          `Then run cc-design-handoff approve --dir ${JSON.stringify(dir)} --phase asset_decomposition --artifact creative.asset-kit.json.`,
        ],
