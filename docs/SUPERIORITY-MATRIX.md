...
| Cockpit product UX | Main surface feels like an AI-native workspace, not a dashboard | VS Code cockpit package, media, webview smoke, deterministic state screenshots, headless interaction proof | More dogfood videos and richer visual-diff thresholds | `cc-cockpit-webview-smoke`, `cc-cockpit-state-proof`, `cc-cockpit-interaction-proof`, `cc-package-cockpit` | Strong |
...
4. **Cockpit visual-diff thresholding**: move from screenshot hash metadata to pass/fail visual-diff thresholds.
