...
|---|---|---|---|---|
| `agent-browser` | Native Rust browser automation CLI (vercel-labs/agent-browser, on PATH at `~/local/bin/`). **Default browser-automation tool as of 2026-05-04** (replaced playwright MCP). Use for clean sites without bot detection. | Browser automation on standard public sites: navigate, click, type, screenshot, snapshot (a11y tree), eval JS, CDP connect, multi-session | Inline Bash returns from each command; chain via `agent-browser skills get core --full` for workflow patterns | Chrome 148 installed at `~/.agent-browser/browsers/`. No MCP; invoke directly via Bash. Skills include electron, slack, exploratory-testing. |
| `camofox-browser` | Stealth REST-API browser server (jo-inc/camofox-browser, port 9377). Camoufox = Firefox fork with C++-engine-level fingerprint spoofing — bypasses Cloudflare / Google bot detection where Chromium can't. | Anti-bot-protected targets, scraping behind CF challenge, sites that fingerprint navigator/WebGL/AudioContext/WebRTC | HTTP JSON via curl; accessibility-snapshot output (~90% smaller than HTML); stable element refs (`e1`, `e2`, ...) | Source `~/code/research/camofox-browser/`. Start manually `cd ~/code/research/camofox-browser && nohup npm start > ~/Library/Logs/camofox-browser.out.log 2>&1 &`. plist at `~/Library/LaunchAgents/bio.aurex.camofox.plist` (NOT auto-loaded — user-controlled). Profiles persist at `~/.camofox/profiles/`. OpenAPI: `/openapi.json`, docs: `/docs`. |
